...
def neighbors(p):
    ...
    in unvisited points and also neighbor must be at most 1 step higher (but
	can be any step lower)"""
	(x, y) = p
	n = []
	...
	for (x_offset, y_offset) in [(-1,0), (0,-1), (1,0), (0,1)]:
		if 0 <= x+x_offset < dimensions[0] and 0 <= y+y_offset < dimensions[1] and \
			...
			if heights[(x+x_offset, y+y_offset)] <= heights[p]+1:
...
dest = 0			# Destination point to reach

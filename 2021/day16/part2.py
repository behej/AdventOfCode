...
class Packet:
	...
	def compute_value(self):
		...
		# Minimum
			...
			val = self.children[0].compute_value()
			for c in self.children:
				val = min(val, c.compute_value())
		# Maximum
		elif self.type == 3:
			val = self.children[0].compute_value()
			for c in self.children:
				val = max(val, c.compute_value())
		# Value
		elif self.type == 4:
			return self.value
		# Greater than
		elif self.type == 5:
			return 1 if (self.children[0].compute_value() > self.children[1].compute_value()) else 0
		# Less than
		elif self.type == 6:
			return 1 if (self.children[0].compute_value() < self.children[1].compute_value()) else 0
		# Equal
		elif self.type == 7:
			val = 1 if (self.children[0].compute_value() == self.children[1].compute_value()) else 0
		else:
			print("Unknwown type")
...
p = Packet(sequence)
print("Computed value is: {}".format(p.compute_value()))